...
CREATE POLICY "Published blog posts are viewable by everyone" ON public.blog_posts FOR SELECT USING (is_published = true OR public.get_current_user_role() = 'admin');
CREATE POLICY "Admins can manage blog posts" ON public.blog_posts FOR ALL USING (public.get_current_user_role() = 'admin');

-- RLS Policies for projects
CREATE POLICY "Projects are viewable by everyone" ON public.projects FOR SELECT USING (true);
CREATE POLICY "Admins can manage projects" ON public.projects FOR ALL USING (public.get_current_user_role() = 'admin');

-- RLS Policies for services  
CREATE POLICY "Active services are viewable by everyone" ON public.services FOR SELECT USING (is_active = true OR public.get_current_user_role() = 'admin');
CREATE POLICY "Admins can manage services" ON public.services FOR ALL USING (public.get_current_user_role() = 'admin');

-- RLS Policies for contact_submissions
CREATE POLICY "Admins can view all contact submissions" ON public.contact_submissions FOR SELECT USING (public.get_current_user_role() = 'admin');
CREATE POLICY "Anyone can create contact submissions" ON public.contact_submissions FOR INSERT WITH CHECK (true);
CREATE POLICY "Admins can update contact submissions" ON public.contact_submissions FOR UPDATE USING (public.get_current_user_role() = 'admin');

-- RLS Policies for security_scans
CREATE POLICY "Users can view their own scans" ON public.security_scans FOR SELECT USING (auth.uid() = user_id OR public.get_current_user_role() = 'admin');
CREATE POLICY "Authenticated users can create scans" ON public.security_scans FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own scans" ON public.security_scans FOR UPDATE USING (auth.uid() = user_id OR public.get_current_user_role() = 'admin');

-- RLS Policies for pricing_plans
CREATE POLICY "Active pricing plans are viewable by everyone" ON public.pricing_plans FOR SELECT USING (is_active = true OR public.get_current_user_role() = 'admin');
CREATE POLICY "Admins can manage pricing plans" ON public.pricing_plans FOR ALL USING (public.get_current_user_role() = 'admin');
